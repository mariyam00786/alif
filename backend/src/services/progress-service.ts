...
import { DailyRecord, ActivityLog } from '../types/database';
...
export function getMonthlyTotal(
  ...
  let total_marks = 0;
  let days_logged = 0;
  const weeks: Map<number, { marks: number; count: number }> = new Map();
...
export function getRank(
  studentId: string,
  allStudentRecords: Record<string, DailyRecord[]>,
  batchStudentIds: string[],
  date: string
): number | null {
...
export function getTopStudents(
  allStudentRecords: Record<string, { records: DailyRecord[]; name: string }>,
  batchStudentIds: string[],
  date: string,
  limit: number = 10
): RankingEntry[] {
