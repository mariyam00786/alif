...
import { calculateActivityMarks } from './scoring-service';
 ...
 */
function validateRecordItem(item: DailyRecordItem): {
  valid: boolean;
  error?: string;
} {
  // Check activity_id
 ...
 * - All items are valid
 */
function validateDailyRecord(record: Partial<DailyRecord>): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
 ...
 */
function calculateDailyStats(items: Record<string, DailyRecordItem>, totalActivities: number) {
 ...
 */
export function submitDailyRecord(record: DailyRecord): DailyRecord {
 ...
 */
export function approveDailyRecord(record: DailyRecord): DailyRecord {
 ...
 */
export function dailyRecordToActivityLogs(dailyRecord: DailyRecord): ActivityLog[] {
 ...
 */
export function activityLogsToDailyRecord(logs: ActivityLog[], totalActivities: number): DailyRecord {
  const items: Record<string, DailyRecordItem> = {};
 ...
 * @returns Score 0-100
 */
export function calculateCompletionScore(record: DailyRecord, maxMarks: number = 100): number {
 ...
 */
export function validateAllActivitiesMarked(
  record: DailyRecord,
