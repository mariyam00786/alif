...
exports.getRatingMarks = getRatingMarks;
exports.calculateActivityMarks = calculateActivityMarks;
exports.calculateActivityScore = calculateActivityScore;
exports.validateScoringRules = validateScoringRules;
exports.calculateDailyTotal = calculateDailyTotal;
exports.getScoringStatistics = getScoringStatistics;
/**
 * Default scoring rules for Phase 1
 * These are fallback values if custom rules are not in database
 */
const DEFAULT_RATING_MARKS = {
    excellent: 10,
    satisfactory: 5,
    needsImprovement: 2,
    notDone: 0,
 ...
 */
function normalizeRatingName(ratingName) {
 ...
 */
function isValidRating(ratingName) {
 ...
 */
function getRatingMarks(ratingName, customRatingMarks) {
    const normalized = normalizeRatingName(ratingName);
    ...
    if (normalized in DEFAULT_RATING_MARKS) {
        return DEFAULT_RATING_MARKS[normalized];
    }
 ...
 * // returns 15
 */
function calculateActivityMarks(ratingName, quantity, quantityMultiplier, maxQuantityBonus, customRatingMarks) {
 ...
 */
function calculateActivityScore(ratingName, quantity, quantityMultiplier, maxQuantityBonus, customRatingMarks) {
    const baseMarks = getRatingMarks(ratingName, customRatingMarks);
    ...
    if (quantity !== undefined && quantity > 0 && quantityMultiplier) {
        const quantityBonus = Math.min(quantity * quantityMultiplier, maxQuantityBonus || quantity * quantityMultiplier);
 ...
 */
function calculateDailyTotal(scores) {
 ...
 */
function getScoringStatistics(ratingDistribution, customRatingMarks) {
