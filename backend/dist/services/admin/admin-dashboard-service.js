...
class AdminDashboardService {
    ...
    async getSnapshot() {
        try {
            ...
            for (const row of (activityLogsResult.data ?? [])) {
            ...
            const studentCountsByBatch = new Map();
            const approvalQueueByBatch = new Map();
            ...
            const categories = new Map((categoriesResult.data ?? []).map((category) => [category.id, category.name ?? 'Uncategorized']));
            ...
            const ratingRules = [
                ...
                ...(scoringRulesResult.data ?? []).map((rule) => ({
                    id: rule.id,
                    ruleKind: 'scoring',
            ...
            const badgeRecipients = new Map();
            for (const row of (studentBadgesResult.data ?? [])) {
    ...
    async assignTeacherToBatch(batchId, teacherId, actor) {
        const client = (0, supabase_1.getSupabaseClient)();
        const { error: deleteError } = await client
            ...
            .eq('batch_id', batchId);
        if (deleteError) {
            throw new http_error_1.HttpError(500, 'Unable to clear the current teacher assignment.', deleteError);
        }
        const { error: insertError } = await client
            .from('teacher_batches')
            .insert({ batch_id: batchId, teacher_id: teacherId });
        if (insertError) {
            throw new http_error_1.HttpError(500, 'Unable to assign the teacher to the batch.', insertError);
        }
    ...
    async setPrimaryRule(ruleId, ruleKind, actor) {
        const client = (0, supabase_1.getSupabaseClient)();
        ...
        if (selectedRuleError) {
            throw new http_error_1.HttpError(500, 'Unable to load the selected rule.', selectedRuleError);
        }
        if (!selectedRule) {
            throw new http_error_1.HttpError(404, 'Rule not found.');
        }
        const activityId = String(selectedRule.activity_id);
        ...
        if (siblingsError) {
            throw new http_error_1.HttpError(500, 'Unable to load sibling rules.', siblingsError);
        }
        const orderedIds = [
            ruleId,
            ...(siblings ?? [])
        ...
        await Promise.all(orderedIds.map((id, index) => client.from(tableName).update({ display_order: index + 1 }).eq('id', id)));
        await this.auditLogService.log({
            actor,
    ...
    describeAudience(targetType, targetId, batchById) {
    ...
    buildReports(students, teachers, batchClasses, notifications, badges) {
