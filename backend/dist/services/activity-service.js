 ...
 * // ]
 */
async function getRatings(activityId) {
 ...
 * // }
 */
async function buildDailyActivityStructure() {
    const categories = await getCategories();
    const result = await Promise.all(categories.map(async (category) => ({
        ...
        icon: category.icon,
        activities: await Promise.all((await getActivitiesByCategory(category.id)).map(async (activity) => ({
            id: activity.id,
            name: activity.name,
 ...
 */
function invalidateActivityCache(activityId) {
